...
describe("TicTacToe", () => {
  describe("(InitializeBoard)", () => {
    it("Board state should be in Ready state", () => {
      ...
      model.board.forEach((arr) =>
        arr.some((val) => assert.strictEqual(val, 0)),
      );
    });
  });
  ...
  describe("(IsValidBoardCell)", () => {
    it("Board cell is not valid", () => {
      ...
      numbers.map((num) =>
        assert.strictEqual(ticktacmodel.isCellValid(num), false),
      );
    });
    ...
    it("Board cell is valid", () => {
      ...
      numbers.map((num) =>
        assert.strictEqual(ticktacmodel.isCellValid(num), true),
      );
    });
  });
