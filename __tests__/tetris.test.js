...
import * as Block from "../src/old-tetris/block.js";
import * as Board from "../src/old-tetris/board.js";
import * as Constants from "../src/old-tetris/constants.js";
...
describe("Tetris", () => {
  describe("(Symbols)", () => {
    it("RotateSymbol", () => {
      ...
      Constants.SYMBOLS.slice(1, 8).forEach((s) =>
        rotateSymbols(s, (block, nTimes) => block.rotate90ClockWise(nTimes)),
      );
      Constants.SYMBOLS.slice(1, 8).forEach((s) =>
        rotateSymbols(s, (block, nTimes) =>
          block.rotate90AntiClockWise(nTimes),
        ),
      );
    });
    ...
    it("GetBoundedSymbolValue", () => {
      ...
      expect(block.boundedMatrix).toEqual([
        [0, 0, 2],
        [2, 2, 2],
      ]);
    ...
    it("RotateBoundedSymbol", () => {
      ...
      const expectedBoundedSymbol = [
        ...
        [
          [2, 2, 2],
          [2, 0, 0],
        ] /* J rotated CW 2 times */,
        [
          [3, 3, 3],
          [0, 0, 3],
        ] /* L rotated CW 2 times */,
        [
          [4, 4],
          [4, 4],
        ] /* O rotated CW 2 times */,
        [
          [0, 5, 5],
          [5, 5, 0],
        ] /* S rotated CW 2 times */,
        [
          [6, 6, 6],
          [0, 6, 0],
        ] /* T rotated CW 2 times */,
        [
          [7, 7, 0],
          [0, 7, 7],
        ] /* Z rotated CW 2 times */,
      ];
      ...
      const rotateSymbCWFunc = (block, nTimes) =>
        block.rotate90ClockWise(nTimes);
      const actualCWBoundedSymbols = Constants.SYMBOLS.slice(1, 8)
        .map((s) => rotateSymbols(s, rotateSymbCWFunc))
        .map((block) => block.boundedMatrix);
      deepEqual(actualCWBoundedSymbols, expectedBoundedSymbol);

      const rotateSymbACWFunc = (block, nTimes) =>
        block.rotate90AntiClockWise(nTimes);
      const actualACWBoundedSymbols = Constants.SYMBOLS.slice(1, 8)
        .map((s) => rotateSymbols(s, rotateSymbACWFunc))
        .map((block) => block.boundedMatrix);
