  ...
  boundedMatrix.forEach((row, ri) => {
    row.forEach((value, ci) => {
      if (value > 0) {
        ...
        context.fillRect(
          block.position.column + ci,
          block.position.row + ri,
          1,
          1,
        );
      }
...
const handleKeyDownEvents = (event) => {
  ...
  switch (event.code) {
    case "ArrowDown":
      if (
        board.isBlockFloat(block) &&
        board.isValidMove(block.position.row + 1, block.position.column, block)
      ) {
    ...
    case "ArrowLeft":
      if (
        board.isValidMove(block.position.row, block.position.column - 1, block)
      ) {
