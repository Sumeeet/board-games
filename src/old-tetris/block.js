import { SYMBOLS_MAP } from "../old-tetris/constants.js";
...
export function Block(row = -1, column = 3, symbol = null) {
  ...
  const isEqual = (block) => {
    ...
    return boundedMatrix.some((row, index) =>
      blockMatrix[index].some((e, i) => row[i] !== e),
    );
  };
  ...
  const transpose = (matrix) => {
    const rowToCol = (row, tmatrix) =>
      row.map((cell, index) => tmatrix[index].push(cell));
    const transMatrix = matrix.map(() => []);
  ...
  (() => {
    ...
    size = {
      width: boundedMatrix[0].length,
      height: boundedMatrix.length,
    };
  })();

  function rotate90ClockWise(nTimes = 1) {
    const block = new Block(position.row, position.column, symbol);
    block.matrix = rotateAux(
      [...this.matrix],
      nTimes,
      compose(transpose, reverse),
    );
    block.boundedMatrix = clipToBounds(block.matrix);
    block.size = {
      width: block.boundedMatrix[0].length,
      height: block.boundedMatrix.length,
    };
    return block;
  }
  ...
  function rotate90AntiClockWise(nTimes = 1) {
    ...
    block.matrix = rotateAux(
      [...this.matrix],
      nTimes,
      compose(transpose, flipVertical),
    );
    block.boundedMatrix = clipToBounds(block.matrix);
    block.size = {
      width: block.boundedMatrix[0].length,
