export class Board {
  constructor(rows = 3, columns = 3) {
    ...
    this.currentBoardState = {
      BoardState: this.boardStates.Ready,
      Player: this.symbol,
      Message: `Player ${this.symbolX} turn`,
    };
  }
  ...
  getSymbol(cellValue) {
    if (cellValue !== this.symbolMap.X && cellValue !== this.symbolMap.O) {
      console.error(
        `Invalid symbol ${cellValue}. Valid symbols are ${this.symbolMap.X} and ${this.symbolMap.O}`,
      );
      return "";
    }
    return Object.keys(this.symbolMap).find(
      (key) => this.symbolMap[key] === cellValue,
    );
  }
  ...
  getSymbolValue(symbol) {
    if (symbol !== this.symbolX && symbol !== this.symbolO) {
      console.error(
        `Invalid symbol ${symbol}. Valid symbols are ${this.symbolX} and ${this.symbolO}`,
      );
      return 0;
    }
  ...
  isWin() {
    const valueTuple = this.tuple.map((tuple) =>
      tuple.map((index) => this.getValueAt(index)),
    );
    const tupleSum = valueTuple.map((tuple) =>
      tuple.reduce((accum, curVal) => accum + curVal),
    );
    if (
      tupleSum.some(
        (val) =>
          val === this.rows * this.symbolMap.O ||
          val === this.rows * this.symbolMap.X,
      )
    ) {
  ...
  isTie() {
    const valueTuple = this.tuple
      .slice(0, this.rows)
      .map((tuple) => tuple.map((index) => this.getValueAt(index)));
    const emptyCellRow = valueTuple.map((tuple) =>
      tuple.some((val) => val === 0),
    );
  ...
  print() {
    ...
    for (let index = 0; index < this.rows; index++) {
      console.log(
        ` ${valueTuple[index].reduce((accum, value) => `${accum} | ${value}`)}   ${index}`,
      );
